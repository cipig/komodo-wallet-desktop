...
namespace atomic_dex
{
    ...
    {
        for (const nlohmann::json& wallet_info : get_wallets_info(name))
        {
...
namespace atomic_dex
{
    ...
    void addressbook_manager::remove_contact_wallet_info(const std::string& name, const std::string& type)
    {
        ...
        wallets_info.erase(std::remove_if(begin(wallets_info), end(wallets_info), [&type](const nlohmann::json& wallet_info)
        {
